 ...
 * -------------------------------------------------------------------------
 * @copyright Copyright (C) 2007-2026 by Thallys Fernandes.
...
class PluginDatainjectionClientInjection
{
    ...
    public static function showUploadFileForm($options = [])
    {
       ...
       //Show file selection
        ...
        if (
            ...
            && PluginDatainjectionNFDocument::isRequiredForItemtype($model->getItemtype())
        ) {
            echo "<tr class='tab_bg_1'>";
            echo "<td>" . __('PDF da NF', 'datainjection') . $size . "</td>";
   ...
   **/
       ...
       //While CSV file is not EOF
        ...
        while ($line != null) {
            ...
            try {
                ...
                self::associateNFToResult($model, $result);
                $DB->commit();
            } catch (Throwable $e) {
                $DB->rollBack();
                ...
                Toolbox::logInFile(
                    'datainjection',
                    sprintf("NF association failed on line %d: %s\n", $injectionline, $e->getMessage())
                );
            }
         ...
         $documentId = (int) PluginDatainjectionSession::getParam('nf_document_id');
         PluginDatainjectionNFDocument::purgeIfOrphan($documentId);
         ...
         Toolbox::logInFile(
             'datainjection',
             sprintf(
                 "Import model #%d finished: %d successful line(s), %d failed/warning line(s), NF document #%d.\n",
                 $model->fields['id'],
                 $successCount,
    ...
    /**
     * Link the batch NF to an asset created by the current line.
     *
     ...
     * @return void
     */
    private static function associateNFToResult(PluginDatainjectionModel $model, array &$result)
    {
        $documentId = (int) PluginDatainjectionSession::getParam('nf_document_id');
        if (
            $documentId <= 0
        ...
        $infocomId = PluginDatainjectionNFDocument::associateAsset(
            $documentId,
            $itemtype,
            $itemsId
        );

        $result['nf_document_id'] = $documentId;
        $result['nf_infocom_id'] = $infocomId;
    }
