   ...
   //If additional informations provided : check if mandatory infos are present
    if (!$model->checkMandatoryFields($_SESSION['datainjection']['infos'])) {
       ...
       //Read file using automatic encoding detection, and do not delete file once readed
        ...
        if ($response) {
            try {
                $requiresNF = PluginDatainjectionNFDocument::isRequiredForItemtype(
                    $model->getItemtype()
                );
                if ($requiresNF) {
                    if (!isset($_FILES['nota_fiscal'])) {
                        throw new RuntimeException(
                            __('A NF PDF is required for this asset import.', 'datainjection')
                        );
                    }

                    PluginDatainjectionNFDocument::purgeIfOrphan(
                        (int) PluginDatainjectionSession::getParam('nf_document_id')
                    );
                    $documentId = PluginDatainjectionNFDocument::createFromUpload(
                        $_FILES['nota_fiscal'],
                        (int) $_SESSION['glpiactive_entity']
                    );
                    PluginDatainjectionSession::setParam('nf_document_id', $documentId);
                } else {
                    PluginDatainjectionNFDocument::purgeIfOrphan(
                        (int) PluginDatainjectionSession::getParam('nf_document_id')
                    );
                    unset($_SESSION['datainjection']['nf_document_id']);
                }
            } catch (Throwable $e) {
                $model->getBackend()->deleteFile();
                Session::addMessageAfterRedirect($e->getMessage(), true, ERROR, true);
