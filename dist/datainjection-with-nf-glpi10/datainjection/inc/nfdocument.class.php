...
/**
 * Manages the NF document shared by all assets created in one import.
 */
final class PluginDatainjectionNFDocument
{
    /**
    ...
    public static function createFromUpload(array $file, $entitiesId)
    {
        ...
        if (!move_uploaded_file($file['tmp_name'], $temporaryPath)) {
            throw new RuntimeException(
                __('The NF PDF could not be moved to the GLPI temporary directory.', 'datainjection')
            );
        }
        ...
        try {
            ...
            $documentId = $document->add([
                'name'                    => sprintf(__('NF - %s', 'datainjection'), $safeName),
            ...
            if (!$documentId) {
                throw new RuntimeException(
                    __('GLPI could not create the NF document.', 'datainjection')
                );
            }
    ...
    /**
     * Associate the NF with an item through the native GLPI relation.
     *
    ...
    /**
     * Associate the NF with an asset and its Infocom, when present.
     *
     * @param int    $documentId NF document ID
    ...
    private static function validatePdf(array $file)
    {
        ...
        if ($error !== UPLOAD_ERR_OK) {
            throw new RuntimeException(
                sprintf(__('NF PDF upload failed with error code %d.', 'datainjection'), $error)
            );
        }
        ...
        if (!$temporaryPath || !is_uploaded_file($temporaryPath)) {
            throw new RuntimeException(__('The uploaded NF PDF is invalid.', 'datainjection'));
        }

        if (strtolower(pathinfo($originalName, PATHINFO_EXTENSION)) !== 'pdf') {
            throw new RuntimeException(__('The NF must have a PDF extension.', 'datainjection'));
        }
        ...
        if ($mime !== 'application/pdf' || $header !== '%PDF-') {
            throw new RuntimeException(__('The NF must be a valid PDF file.', 'datainjection'));
        }
