...
class PluginDatainjectionClientInjection
{
    ...
    public static function showUploadFileForm($options = [])
    {
       ...
       //Show file selection
        ...
        if (
            $confirm !== 'creation'
            &&
            $model->getFromDB($options['models_id'])
            && PluginDatainjectionInvoiceDocument::isRequiredForItemtype($model->getItemtype())
        ) {
            echo "<tr class='tab_bg_1'>";
            echo "<td>" . __('Invoice PDF', 'datainjection') . $size . "</td>";
            echo "<td><input type='file' name='nota_fiscal' accept='application/pdf,.pdf' required>";
            echo "</td></tr>";
        }
   ...
   **/
    public static function processInjection(PluginDatainjectionModel $model, $entities_id)
    {
       ...
       //While CSV file is not EOF
        ...
        while ($line != null) {
            ...
            $DB->beginTransaction();
            try {
                $result = $engine->injectLine($line[0], $injectionline);
                self::associateInvoiceToResult($model, $result);
                $DB->commit();
            } catch (Throwable $e) {
                $DB->rollBack();
                $engine->addLineInError($line[0]);

                $result = [
                    'line'    => $injectionline,
                    'status'  => PluginDatainjectionCommonInjectionLib::FAILED,
                    'type'    => PluginDatainjectionCommonInjectionLib::IMPORT_ADD,
                    'message' => $e->getMessage(),
                    PluginDatainjectionCommonInjectionLib::ACTION_CHECK => [
                        'status' => PluginDatainjectionCommonInjectionLib::FAILED,
                        [
                ...
                Toolbox::logInFile(
                    'datainjection',
                    sprintf("Invoice association failed on line %d: %s\n", $injectionline, $e->getMessage())
                );
            }
            $clientinjection->results[] = $result;
         ...
         $documentId = (int) PluginDatainjectionSession::getParam('invoice_document_id');
         PluginDatainjectionInvoiceDocument::purgeIfOrphan($documentId);

         $successCount = count(array_filter(
             $clientinjection->results,
             static function ($result) {
                 return ($result['status'] ?? null) === PluginDatainjectionCommonInjectionLib::SUCCESS;
             }
         ));
         Toolbox::logInFile(
             'datainjection',
             sprintf(
                 "Import model #%d finished: %d successful line(s), %d failed/warning line(s), invoice document #%d.\n",
                 $model->fields['id'],
                 $successCount,
                 count($clientinjection->results) - $successCount,
                 $documentId
             )
         );
    ...
    /**
     * Link the batch invoice to an asset created by the current line.
     *
     * @param PluginDatainjectionModel $model  Import model
     * @param array                    $result Injection result
     *
     * @return void
     */
    private static function associateInvoiceToResult(PluginDatainjectionModel $model, array &$result)
    {
        $documentId = (int) PluginDatainjectionSession::getParam('invoice_document_id');
        if (
            $documentId <= 0
        ...
        if (
            ...
            || ($result['type'] ?? null) !== PluginDatainjectionCommonInjectionLib::IMPORT_ADD
        ) {
            return;
        }

        $itemtype = $model->getItemtype();
        $itemsId = (int) ($result[$itemtype] ?? 0);
        if ($itemsId <= 0) {
            throw new RuntimeException(
                __('The injection succeeded but did not return the created asset ID.', 'datainjection')
            );
        }

        $infocomId = PluginDatainjectionInvoiceDocument::associateAsset(
            $documentId,
            $itemtype,
            $itemsId
        );

        $result['invoice_document_id'] = $documentId;
        $result['invoice_infocom_id'] = $infocomId;
    }
