   ...
   //If additional informations provided : check if mandatory infos are present
    if (!$model->checkMandatoryFields($_SESSION['datainjection']['infos'])) {
       ...
       //Read file using automatic encoding detection, and do not delete file once readed
        ...
        if ($response) {
            try {
                $requiresInvoice = PluginDatainjectionInvoiceDocument::isRequiredForItemtype(
                    $model->getItemtype()
                );
                if ($requiresInvoice) {
                    if (!isset($_FILES['nota_fiscal'])) {
                        throw new RuntimeException(
                            __('An invoice PDF is required for this asset import.', 'datainjection')
                        );
                    }

                    PluginDatainjectionInvoiceDocument::purgeIfOrphan(
                        (int) PluginDatainjectionSession::getParam('invoice_document_id')
                    );
                    $documentId = PluginDatainjectionInvoiceDocument::createFromUpload(
                        $_FILES['nota_fiscal'],
                        (int) $_SESSION['glpiactive_entity']
                    );
                    PluginDatainjectionSession::setParam('invoice_document_id', $documentId);
                } else {
                    PluginDatainjectionInvoiceDocument::purgeIfOrphan(
                        (int) PluginDatainjectionSession::getParam('invoice_document_id')
                    );
                    unset($_SESSION['datainjection']['invoice_document_id']);
                }
            } catch (Throwable $e) {
